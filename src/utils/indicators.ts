...
export function emaSeries(values:number[], p:number, bars:Bar[]){
  ...
  return e.map((v,i)=> ({ time: Math.floor(bars[i].t/1000 as UTCTimestamp) as UTCTimestamp, value: v }))
}
...
export function rsiSeriesCalc(values:number[], period:number, bars:Bar[]){
  ...
  return a.map((v,i)=> v==null? { time: Math.floor(bars[i].t/1000 as UTCTimestamp) as UTCTimestamp, value: 50 } : { time: Math.floor(bars[i].t/1000 as UTCTimestamp) as UTCTimestamp, value: v })
}
...
export function vwapSeries(bars:Bar[]){
  const v = vwap(bars)
  return v.map((val,i)=> ({ time: Math.floor(bars[i].t/1000 as UTCTimestamp) as UTCTimestamp, value: val }))
}
