...
export default function App(){
  ...
  useEffect(()=>{
    ...
    seriesC.current = main.addCandlestickSeries({ 
      upColor:'#18c964', downColor:'#ff4d4f',
      wickUpColor:'#18c964', wickDownColor:'#ff4d4f',
      borderVisible:false
    })
    seriesV.current = main.addHistogramSeries({ priceScaleId: '', priceFormat: { type: 'volume' }, color:'#2a3544' })
    main.priceScale('').applyOptions({ scaleMargins:{ top:0.85, bottom:0 } })
    ...
    rsiS.current  = rsi.addLineSeries({ color:'#9aa4b2', lineWidth: 2 })
    chartApi.current = main; rsiApi.current = rsi

    main.timeScale().subscribeVisibleTimeRangeChange((tr)=> { if (tr) rsi.timeScale().setVisibleRange(tr) })
    rsi.timeScale().subscribeVisibleTimeRangeChange((tr)=> { if (tr) main.timeScale().setVisibleRange(tr) })
  ...
  useEffect(()=>{
    ...
    seriesC.current.setData(arr.map(k=>({ time: Math.floor(k.t/1000) as UTCTimestamp, open:k.o, high:k.h, low:k.l, close:k.c })))
    seriesV.current?.setData(arr.map(k=>({ time: Math.floor(k.t/1000) as UTCTimestamp, value:k.v, color: k.c>=k.o? '#1e8b5f':'#8b2f33' })))
  ...
  return (
    <div className="app">
      ...
      <footer>
        <button className="btn long" onClick={()=> { setPopupMsg('LONG'); setShowPopup(true); setTimeout(()=>setShowPopup(false),1500) }}>LONG</button>
        <button className="btn short" onClick={()=> { setPopupMsg('SHORT'); setShowPopup(true); setTimeout(()=>setShowPopup(false),1500) }}>SHORT</button>
        <button className="btn close" onClick={()=> { setPopupMsg('CLOSE'); setShowPopup(true); setTimeout(()=>setShowPopup(false),1500) }}>CLOSE</button>
      </footer>
